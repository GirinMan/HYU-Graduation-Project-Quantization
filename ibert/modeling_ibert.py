...
class IBertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.quant_mode = config.quant_mode_embedding
...
class IBertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.quant_mode = config.quant_mode_attention
        self.weight_bit = 8
        self.bias_bit = 32
        self.act_bit = 8
...
class IBertAttention(nn.Module):
    def __init__(self, config, layer_num):
        super().__init__()
        self.quant_mode = config.quant_mode_attention or config.quant_mode_layer[layer_num]
...
class IBertIntermediate(nn.Module):
    def __init__(self, config, layer_num):
        super().__init__()
        self.quant_mode = config.quant_mode_intermediate or config.quant_mode_layer[layer_num]
        self.act_bit = 8
        self.weight_bit = 8
        self.bias_bit = 32
        self.dense = QuantLinear(
...
class IBertOutput(nn.Module):
    def __init__(self, config, layer_num):
        super().__init__()
        self.quant_mode = config.quant_mode_layer[layer_num]
        self.act_bit = 8
        self.weight_bit = 8
        self.bias_bit = 32
        self.ln_input_bit = 22
...
class IBertLayer(nn.Module):
    def __init__(self, config, layer_num):
        ...
        self.seq_len_dim = 1
        self.attention = IBertAttention(config,layer_num)
        self.intermediate = IBertIntermediate(config,layer_num)
        self.output = IBertOutput(config,layer_num)
...
class IBertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([IBertLayer(config,i) for i in range(config.num_hidden_layers)])
